# component-operator - AI Agent Guide
...
This repo's `PROJECT` pins the plugin's `output` to `.` (repo root), so the
committed chart lives at `chart/`, not the plugin's `dist/chart` default —
always pass `--output-dir=.` when regenerating:

```bash
kubebuilder edit --plugins=helm/v2-alpha --output-dir=.  # Generates chart/ (this repo's convention)
```
...
```bash
helm install my-release ./chart/ --namespace <ns> --create-namespace
```
...
2. Re-run: `kubebuilder edit --plugins=helm/v2-alpha --force --output-dir=.`
